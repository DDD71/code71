...
class DQN(object):
    def __init__(self):
        ...
        self.eval_net , self.target_net = Net(N_STATES,50,N_ACTIONS),Net(N_STATES,50,N_ACTIONS)
    ...
    def update_target_net(self):
        if self.learn_step_counter % self.target_update_freq == 0:   # 每n step更新一次target net
    ...
    def training(self):
        ...
        b_s,b_a,b_r,b_s_ = self.get_samples()

        ### Nature DQN
        # q_eval = self.eval_net(b_s).gather(1, b_a)  # shape (batch, 1)
        # q_next = self.target_net(b_s_).max(1)[0].reshape(self.batch_size, 1).detach()     # detach from graph, don't backpropagate
        # q_target = b_r + self.gamma * q_next   # shape (batch, 1)

        # # Double DQN   
        # #q_eval w.r.t the action in experience  
        q_eval = self.eval_net(b_s).gather(1, b_a)  # shape (batch, 1)
        next_action = self.eval_net(b_s_).max(1)[1].reshape(self.batch_size, 1)   # the different from nature DQN 先从q_eval net 中选出价值最大的动作
        ...
        loss.backward()
        self.optimizer.step()
        self.loss_value = loss.item()  #record loss
...
if __name__ == "__main__": 
    ...
    for i_episode in range(dqn.max_episode):
        ...
        while True:
            ...
            r = dqn.get_reward(s_)  # modify the reward to learn, speed up convergence
